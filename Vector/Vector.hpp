 ...
 */
template<typename T>
Vector<T>::~Vector() { delete[] _values; }
...
/**
 * @breif Iterator erase at position.
 * @tparam T
 */
template<typename T>
iterator<T> Vector<T>::erase(it position) {
    std::size_t i = 0;
    auto it = (*this).begin();
    for (it; it != position; it++, i++);
    for (auto it = position + 1; it != (*this).end(); it++, i++)
 ...
 */
template<typename T>
T &Vector<T>::operator[](int i) { return _values[i]; }
 ...
 */
template<typename T>
const T *Vector<T>::data() { return _values; }